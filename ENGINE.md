...
(server-rendered Go + htmx; maintained compat stylesheets, cssgen-linted). This
...
block's DOM to restyle it; override in `components.compat.css` (our layer)
instead. Both stylesheets are maintained sources: `classic.compat.css` is the
theme, derived from the TheLCARS.com template (attribution in LICENSE.md; the
upstream file is no longer vendored), and `components.compat.css` is our
component layer. Every rule must stay old-browser compatible — no
clamp()/min()/max(), :has(), logical properties, flex `gap` (child margins;
`grid-gap` for grids), or text-box; `go run ./tools/cssgen` validates both.
...
- **The compat CSS is the source.** Pages are served with `*.compat.css`,
  hand-authored for ~2017 browsers (see decisions/0008): @media breakpoints
  instead of clamp()/min()/max(), physical properties instead of logical,
  sibling margins instead of flex `gap`, `grid-gap` instead of grid `gap`.
  After any CSS change run `go run ./tools/cssgen` — it fails the build if
  modern syntax slipped in.
- **Use the spacing tokens, not ad-hoc gaps.** `var(--gap-sm)` (0.25rem)
